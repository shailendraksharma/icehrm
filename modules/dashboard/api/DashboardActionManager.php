...
class DashboardActionManager extends SubActionManager{
	...
	public function getPendingLeaves($req){
		...
		$pendingLeaves = $leave->Find("status = ? and employee = ?",array("Pending", $this->getCurrentEmployeeId()));
	...
	public function getLastTimeSheetHours($req){
		...
		$timeSheet->Load("employee = ? order by date_end desc limit 1",array($this->getCurrentEmployeeId()));
	...
	public function getEmployeeActiveProjects($req){
		...
		$projects = $project->Find("employee = ? and status =?",array($this->getCurrentEmployeeId(),'Current'));
